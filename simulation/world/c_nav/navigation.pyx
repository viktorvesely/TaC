...
cdef double heuristic(int x1, int y1, int x2, int y2) nogil:
...
cdef void heap_push(HeapItem* heap, int* heap_size_ptr, HeapItem item) nogil:
...
cdef HeapItem heap_pop(HeapItem* heap, int* heap_size_ptr) nogil:
    cdef int i = 0
...
def astar(int from_i, int from_j, int to_i, int to_j, double[:, :] walls):
    ...
    cdef cnp.ndarray[np.int32, ndim=2] offsets = np.zeros((8, 2), dtype=np.int32)
